...
String[] labels = new String[] {"ABC","DEF","GHI","JKL","SPACE","MNO","PQRS","TUV","WXYZ"};
int selecting = -1;
...
void drawDesign() {
  ...
  if (selecting == 6) {//PQRS  bottom left
    drawButton(selecting, labels[selecting].substring(0,1));
    fill(255);
    drawButton(selecting-3, labels[selecting].substring(1,2));
    fill(255);
  ...
  if (selecting == 8) {//WXYZ  bottom right
    drawButton(selecting, labels[selecting].substring(0,1));
    fill(255);
...
void drawButtons() {
  ...
  for (int i=0; i<9; i++) {
    if (i==4) {fill(150); textSize(10);}
    else {fill(220); textSize(12);}
